...
@tanjun.with_channel_slash_option(
    "channel", "channel to limit opinions to", default=None
)
@tanjun.as_slash_command(
    "rankedopinion", "Find out what a channel/server thinks of a topic"
)
async def rankedopinion(
    ...
    for member_id in top_members:
        ...
        messages = db.get_messages(
            member=member_id, guild=ctx.guild_id, channel=channel, text=topic
        )
        ...
        await ctx.edit_initial_response(
            f"{prefix_str}'s opinions on *{topic}*:\n{final_output}"
        )

    if output is None or len(output) == 0:
        await ctx.edit_initial_response(
            f"{prefix_str} doesn't have an opinion on *{topic}*"
        )
    else:
        final_output = reversed(sorted(output, key=lambda tup: tup[0]))
        final_output = [f"{idx}. {e[1]}" for idx, e in enumerate(final_output)]
