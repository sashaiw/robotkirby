...
def is_chron_check(message):
    return message.author.id == "89398547308380160"

def is_chron():
    return commands.check(lambda ctx: is_chron_check(ctx.message))

def is_admin_check(message):
    return message.author.id in ("89398547308380160", "89412220898803712") or message.author.id == message.server.owner.id
